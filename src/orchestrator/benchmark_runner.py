...
class BenchmarkOrchestrator:
    ...
    def run_suite(self, config_ids: list[str] | None = None, session_id: str | None = None):
        ...
        if self.n_runs > 1:
            ...
            # Generate HTML Dashboard with aggregated data
            try:
                out_path = os.path.join(self.results_dir, f"dashboard_{timestamp}.html")
                from src.features.multi_run import aggregate_session
                agg_data = aggregate_session(self.results_dir, session_id)
                # Convert dict to list of records for dashboard_builder
                records = []
                for cid, stats in agg_data.items():
                    r = stats.copy()
                    r["config_id"] = cid
                    r["config_name"] = cid # fallback
                    records.append(r)
                
                generate_dashboard(self.results_dir, out_path, aggregated_data=records)
            ...
            # Generate HTML Dashboard (Single run)
            try:
                out_path = os.path.join(self.results_dir, f"dashboard_{timestamp}.html")
