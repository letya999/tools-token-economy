...
## Результаты — прогон 2026-05-30

20 из 21 конфигов прошли (95%). Ключевые выводы:

| Конфиг | Инструменты | Токены | Цена $ | SPT | Waste% |
|---|---|---|---|---|---|
...
> **SPT** = Success Per 1K Tokens (чем выше — тем эффективнее). **Waste%** — доля контекста, прочитанного, но не использованного в итоговом патче.
> `03_gemini_like` (read\_all + repo\_map) упал — взрыв контекста при чтении всего репозитория.

**Главный вывод:** точечный поиск (`git_grep`, `rg`, `grep`) в разы эффективнее стратегий с массовым чтением. Подход Claude Code (glob+rg) в 58 раз токен-эффективнее, чем read\_all, при стоимости в 28 раз ниже.
